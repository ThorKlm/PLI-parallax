...
for f in sorted(glob.glob(f"{D}/crystal_labels/crystal_labels_*.npz")):
    ...
    del d
ids=[l.strip() for l in open(f"{D}/msa_sample_ids.txt") if l.strip()]
SID=[];RM=[];POC=[]
done=miss=fail=0
for s in ids:
    pp=f"{D}/boltz_out_msa/boltz_results_boltz_in_msa/predictions/{s}/{s}_model_0.pdb"
    if not os.path.exists(pp): pp=f"{D}/boltz_out_msa/predictions/{s}/{s}_model_0.pdb"
...
np.savez_compressed(f"{D}/bisy/bisy_boltz2_msa.npz",system_id=np.array(SID),ligand_rmsd=np.array(RM,np.float32),pocket_resid=np.array(POC,np.float32))
