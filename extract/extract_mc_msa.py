"""Extract residue-ligand distance labels from the multimer MSA Boltz-2 run.

Adapted from extract_mc_empty.py, which is itself the multimer adaptation of
extract_exp_msa.py.  The extraction logic -- and every verification it performs
-- is carried over unchanged; only the arm-specific inputs differ:

  1. Predictions come from mc_msa_fold/out/boltz_results_boltz_in_mc_msa_cached/,
     the MSA arm.  Same Boltz build and sampling settings as the empty arm
     (2.2.1, seed 42, recycling 3, sampling 50, 1 diffusion sample, mmcif); the
     only difference is that the protein chains carry a cached ColabFold MSA.
  2. The cohort is the MSA arm's own launched set -- the 1814 YAMLs in
     in/boltz_in_mc_msa_cached, i.e. the systems whose MSAs were already cached
     -- not the 3655-system empty-MSA cohort.  It is a strict subset of it.
     Systems the run did not finish are recorded as `missing`, not skipped.
  3. Only the mmCIF is read.  The PAE and PDE npz files were deleted after
     archiving, so nothing here may depend on them (the source script did not
     either -- confidence/plddt outputs are never opened).

Alignment handling is identical and deliberately so.  These are true multimers:
Boltz emits one chain per crystal chain and may emit them in a permuted order.
The deposited res_row enumerates standard-AA CA-bearing residues across all
chains in CRYSTAL mmCIF chain order, so every predicted residue is remapped onto
that enumeration via the chain schema's chain_res_offsets before contacts are
written.  Chain identity is resolved by the mmCIF chain NAME, never by position
(Boltz preserves the input chain id even when it reorders), and every chain is
checked against the schema's per-chain length and sequence before any label is
emitted -- a system that fails is recorded as a failure, never silently written
with a wrong enumeration.

Two reordering flags are written.  `boltz_reordered` is the manifest's
`boltz_order_is_identity` negated, which is what the empty arm's npz carries, so
the field means the same thing in both arms.  `cif_reordered` is the order
observed in *this* arm's mmCIF, derived from the chain names alone -- a fact
about these predictions rather than a manifest lookup.

Output teacher tag is boltz2_mc_msa so it cannot collide with the deposited
boltz2 / boltz2_msa / boltz2_r3empty labels or with boltz2_mc_empty.

Run: python extract_mc_msa.py <shard> <nshard>
"""
...
PRED = "/workspace/mc_msa_fold/out/boltz_results_boltz_in_mc_msa_cached/predictions"
YAML = "/workspace/mc_msa_fold/in/boltz_in_mc_msa_cached"
...
STAT = "/workspace/reports/mc_msa_cached_systems.txt"
...
def cohort():
    """The launched cohort: one entry per input YAML.

    The MSA arm has no per-system status TSV (the empty arm's STAT file), so the
    cohort is the input directory itself -- the systems actually handed to Boltz.
    reports/mc_msa_cached_systems.txt is the list the run was built from; the two
    must agree, and a disagreement means the run's inputs are not the ones this
    extractor thinks it is describing, so it is fatal rather than reconciled.
    """
    yam = sorted(os.path.basename(p)[:-5] for p in glob.glob(f"{YAML}/*.yaml"))
    listed = sorted(open(STAT).read().split())
    if yam != listed:
        raise SystemExit(f"cohort mismatch: {len(yam)} yaml vs {len(listed)} in "
                         f"{STAT}; only_yaml={sorted(set(yam)-set(listed))[:5]} "
                         f"only_listed={sorted(set(listed)-set(yam))[:5]}")
    return yam
...
def crystal_remap(chains, emitted_ids, counts, offsets, seqs):
    ...
    Returns (ca, heavy, remap, cif_order) where remap[pred_row] is the deposited
    res_row and cif_order lists the schema chain index of each emitted chain, in
    mmCIF order. Raises AlignError if the predicted chain set, any chain length,
    or any chain sequence disagrees with the crystal schema.
    """
    ...
    return np.array(ca, np.float32), heavy, remap, cif_order
...
def main():
    ...
    for sid in mine:
        ...
        try:
            ...
            ca, heavy, remap, cif_order = crystal_remap(
        ...
        CIFREORD.append(cif_order != sorted(cif_order))
    ...
    np.savez_compressed(f"{OUT}/exp_{SRC}_{shard}.npz",
        ...
        cif_reordered=np.array(CIFREORD, bool),
    ...
    print(f"{SRC} shard {shard}: done={done} miss={miss} fail={fail} "
          f"reordered={sum(REORD)} cif_reordered={sum(CIFREORD)} contacts={len(C_res)}")
