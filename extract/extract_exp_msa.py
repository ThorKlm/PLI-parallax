...
def main():
    ...
    def locate(sid):
        safe=sid.replace('.','_')
        if teacher=="boltz":
            for base in ["/workspace/datasets/experimental_expansion/boltz_out_msa/boltz_results_boltz_in_msa/predictions",
                         "/workspace/datasets/experimental_expansion/boltz_out_msa2/boltz_results_boltz_in_msa2/predictions"]:
                p=f"{base}/{safe}/{safe}_model_0.pdb"
    ...
    src="boltz2_msa" if teacher=="boltz" else "chai1"
